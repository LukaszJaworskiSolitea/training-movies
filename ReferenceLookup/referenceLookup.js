            ...
            var ReferenceLookupBindingHandler = /** @class */ (function () {
                ...
                ReferenceLookupBindingHandler.prototype.init = function (element, valueAccessor, allBindings, viewModelDeprecated, bindingContext) {
                    ...
                    var widget = $(element).autocomplete({
                        ...
                        messages: {
                            noResults: function () {
                                return vm.dataContext.$localize('ReferenceLookup$$noResultsFound');
                            },
                            results: function (count) {
                                var items = vm.dataContext.$localize('ReferenceLookup$$numberOfFoundItems');
                                var chooseOptText = vm.dataContext.$localize('ReferenceLookup$$chooseOptionText');
                                var elementValue = $(element).val().trim();
                                var tabKeyText = elementValue.length > 0 ? vm.dataContext.$localize('ReferenceLookup$$tabSelectsTheFirstOption') : '';
                    ...
                    $(element).blur(function (event, ui) {
                        ...
                        if (elementValue.length === 0) {
                            ...
                            if (cachedItem_1) {
                                if (widget._itemIsSelectable(cachedItem_1)) {
                ...
                TaricLookupBindingHandler.prototype.init = function (element, valueAccessor, allBindings, viewModelDeprecated, bindingContext) {
                    ...
                    widget._renderMenu = function (ul, items) {
                        $.each(items, function (index, item) {
                            ...
                            if (item.disabled) {
                                li.attr('aria-label', item.label + " (" + vm.dataContext.$localize('ReferenceLookup$$disabled') + ")");
                            }
                        });
                    };
                };
        ...
        (function (referenceLookup) {
            ...
            var TaricLookupViewModel = /** @class */ (function () {
                ...
                TaricLookupViewModel.prototype.getLookupData = function (startString, attributeName) {
                    ...
                    return this.dataApiObject.getLookupData(startString, attributeName, this.useContains, this.expectLinebreaksInValues, null, this.resultSorting, ko.unwrap(this.staticODataFilter))
                        .then(function (values) {
                        _this.itemsCache = { startString: startString, values: values };
                        return values;
                    });
